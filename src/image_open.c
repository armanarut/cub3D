...
void	init_sprite_file(void)
{
	...
	while (++i < 6)
	{
		...
		if (!temp.spr_image[i].img)
			put_error("Invalid file of sprite");
...
void	open_wall_image(void)
{
	...
	if (!temp.image[0].img || !temp.image[1].img
		|| !temp.image[2].img || !temp.image[3].img)
			put_error("Invalid file of wall");
